...
class Response implements ResponseInterface
{
     ...
     * @param StreamInterface|string $reason Reason phrase (when empty a default will be used based on the status code)
     */
    public function __construct(int $status = 200, array $headers = [], StreamInterface|string $body = '', string $version = '1.1', string $reason = '')
    {
        ...
        $this->reasonPhrase = '' === $reason && isset(self::PHRASES[$this->statusCode]) ? self::PHRASES[$status] : $reason;
    ...
    public static function json(
        ...
        // Set the default HTTP status code if none is provided
        $status = $code ?? 200;

        // Determine JSON encoding options
        ...
        if (is_string($body)) {
            ...
            $bodyContent = $body; // Valid JSON string can be used directly
        } else {
            $bodyContent = json_encode($body, $jsonOptions);
        }

        // Merge custom headers with the default JSON content type
        $defaultHeaders = ['Content-Type' => 'application/json'];
        $responseHeaders = array_merge($defaultHeaders, $headers);

        // Return the response
        return new self($status, $responseHeaders, Stream::create($bodyContent));
    }
    ...
    public static function unavailable(string $message = 'Service Unavailable'): Response
    {
